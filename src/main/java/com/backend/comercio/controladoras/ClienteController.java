...
import com.backend.comercio.servicios.ClienteService;

import static com.backend.comercio.Constantes.CLIENTECONTROLLER;
...
public class ClienteController {
    ...
    public ResponseEntity<?> actualizar(@RequestBody @Valid Cliente _cliente) {
    	logger.info(LOGMETHOD+Thread.currentThread().getStackTrace()[1].getMethodName()+LOGCLASS+this.getClass().getSimpleName());
    ...
    @GetMapping(value = "/obtenerporcelular/{celular}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> obtenerPorCelular(@PathVariable("celular") String celular) {
    	logger.info(LOGMETHOD+Thread.currentThread().getStackTrace()[1].getMethodName()+LOGCLASS+this.getClass().getSimpleName());
        Optional<Cliente> cliente=servicio.obtenerPorCelular(celular);
        return new ResponseEntity<>(cliente, HttpStatus.OK);
    }
}
